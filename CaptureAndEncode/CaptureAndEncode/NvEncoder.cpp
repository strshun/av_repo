...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, GUID codecGuid, GUID presetGuid, NV_ENC_TUNING_INFO tuningInfo)
{
    ...
    pIntializeParams->frameRateNum = 20;
...
void NvEncoder::EncodeFrame(std::vector<NvEncOutputFrame> &frames, NV_ENC_PIC_PARAMS *pPicParams)
{
	frames.clear();
	if (!IsHWEncoderInitialized())
	{
	...
	{
		m_iToSend++;
		GetEncodedPacket(m_vBitstreamOutputBuffer, frames, true);
	}
	else
	{
		NVENC_THROW_ERROR("nvEncEncodePicture API failed", nvStatus);
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, std::vector<NvEncOutputFrame> &frames, bool bOutputDelay)
{
	unsigned i = 0;
    int iEnd = bOutputDelay ? m_iToSend - m_nOutputDelay : m_iToSend;
    for (; m_iGot < iEnd; m_iGot++)
    {
		...
		NvEncOutputFrame frame = {};
		frame.width = m_nWidth;
		frame.height = m_nHeight;
		frame.frameType = lockBitstreamData.pictureType;
		frame.index = lockBitstreamData.frameIdx;
		frame.size = lockBitstreamData.bitstreamSizeInBytes;

		uint8_t *pData = (uint8_t *)lockBitstreamData.bitstreamBufferPtr;
		frame.frameBuffer.clear();
		frame.frameBuffer.insert(frame.frameBuffer.end(), &pData[0], &pData[lockBitstreamData.bitstreamSizeInBytes]);
