...
SnapEvent NotSavePicQueue[20000];
int offLineCount = 0;
...
int initFFMPEG(const char* rtsp_url, const char* picName, int cur_index,std::ofstream* ofile){
    ...
    if((ret=avformat_open_input(&fmt_ctx,rtsp_url,0,&avdic))<0)
    {
		...
		//memcpy(NotSavePicQueue[offLineCount].cameraID,RtspQueue[cur_index].cameraID, 100);
		//memcpy(NotSavePicQueue[offLineCount].picName,RtspQueue[cur_index].picName, 400);
		ofile->write(RtspQueue[cur_index].cameraID,strlen(RtspQueue[cur_index].cameraID));
		(*ofile)<< ",";
		ofile->flush();
		offLineCount++;
		av_dict_free(&avdic);
...
void getRtspUrl(char* cameraID,int CameNum){
	    char req[1024]={0};
		char reData[500]={0};
		int i=0,j=0, recvlen=0;
...
void run(int start, int end){
	char fileName[50]={0};
	sprintf(fileName, "./notSavePic%d.txt",start);
    ...
    {
		getRtspUrl(RtspQueue[j].cameraID,j);
        initFFMPEG(RtspQueue[j].rtspurl, RtspQueue[j].picName,j, &outFile);
		printf("offlineDevCount %d picName %s\n", offLineCount,RtspQueue[j].picName);
    }
	outFile.close();
	printf("current thread done");
}
